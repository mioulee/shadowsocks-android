...
//            val profile = Core.currentProfile
            ...
            var password = DataStore.privateStore.getString(Key.password)
            var method = DataStore.privateStore.getString(Key.method)
            profile?.host = DataStore.privateStore.getString(Key.host)?:""
            profile?.password = if(password.isNullOrEmpty()) "banana" else password?:"banana"
            profile?.remotePort = DataStore.privateStore.getInt(Key.remotePort)?:1080
            profile?.method = if (method.isNullOrEmpty()) "chacha20-ietf" else method?:"chacha20-ietf"
            profile?.bypass = true
            profile?.route = Acl.BYPASS_LAN_CHN
            this as Context
            if (profile == null) {
                // gracefully shutdown: https://stackoverflow.com/q/47337857/2245107
                data.notification = createNotification("Orange智能高速线路")
